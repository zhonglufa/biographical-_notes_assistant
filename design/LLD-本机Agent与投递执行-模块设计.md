...
> 本文件为交付级 LLD v1.2（v1.1 修复 F-1/F-2/F-3：幂等四元组、心跳 fail-closed 阈值、验证码状态映射；v1.2 修复评审 F-4~F-14 + 收口 R-09：warm-up 份数模型偏差标注、抖动 8s+贝塞尔、孤儿进程回收、SQLite 完整性/受限安全模式、§6.15 误导呼应澄清、IPC 帧/鉴权/版本、§14 ≥5 时序补第 5 条、PRD 锚点更正、TC 占位声明、PoC 阈值提议标注、退避 base/cap 回声；R-09 失效检测 SLA + 单平台降级已落地）。下一步：先跑 §10 PoC 双实验。
