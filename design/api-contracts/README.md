...
源契约经 `../contracts/validate_contracts.py` 校验器闭环（schema 66 / 注册表 6 全绿；以 `python ../contracts/validate_contracts.py` 实测为准）。
...
## 覆盖率（41 个 operation / 75 个 component）

| 层 | 端点 | 契约严格度 | 来源 |
| --- | --- | --- | --- |
| **A 层 · 外部 API** | A01–A25（25 个 REST 端点） | **严格 schema 已落盘 `../contracts/`（request/response *.schema.json）并经双闸门校验**（registry `contractStatus=fully-detailed`，v3.29 闭环）；本 openapi.json 为**投影视图**（operation 级 `x-contract-status` 反映 registry 状态，内联 body 为字段大纲投影，`x-ref` 指向严格契约） | `external-api.registry.json` |
...
```
生成器自带双闸门自检：① 全部 `$ref` 必须可解析（本版 82 处全解析）；② 无 `nullable` 残留（draft 2020-12 → OAS 3.1 联合类型转换）。任一失败即报错退出。
...
- A 层端点（operation 级已 `fully-detailed`，内联 body 为字段大纲投影）与面试域 / device 端点返回投影结构的 Mock（字段类型 best-effort，足以驱动前端联调）；A 层严格 Mock 以 `x-ref` 指向的 `../contracts/` schema 为准。
...
本 `openapi.json` 是其**导出视图（投影）**：A 层严格 schema 已存在于 `../contracts/`（registry `fully-detailed`，LLD 细化已于 v3.29 闭环，无「待细化」残留），本文件 operation 级标注其状态、内联 body 为字段大纲投影（标 `x-contract-status` 仅表示投影粒度）；B 层为严格投影（直引 b0X schema + 示例）。
若源契约更新，重跑 `gen_openapi.py` 即可同步；不要手工改 `openapi.json`（会被覆盖）。

## 已知边界（诚实登记，非未定义）

- A 层 25 端点严格 schema 已落盘 `../contracts/` 并经双闸门校验（registry `contractStatus=fully-detailed`，LLD 细化已于 v3.29 闭环，无「待细化」残留）；openapi.json 内联 body 为字段大纲投影（标 `x-contract-status` 表示投影粒度），**权威逐字段 schema 见各 operation `x-ref`**，请勿以 openapi 内联 body 作为严格契约来源。
- `interviewEvaluation` 等引用类型已解析为严格组件；A 层 `jobStub`/`job`/`hrStatus` 等 outlined 引用以轻量 stub 组件兜底（保证 `$ref` 可解析，标注未细化——这些 $ref 目标类型本身尚未升 strict，与 A 层 operation 已 fully-detailed 不矛盾）。
