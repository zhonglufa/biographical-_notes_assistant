...
> 2026-08-16 · **v3.30（测试计划文档收口 #169）**：架构师自主推进 P2 文档交付第一块，闭合审查报告 P2「测试计划文档」。新建 `design/测试计划.md` v1.0：七层测试分层（单元/契约/集成/端到端/性能/安全/混沌）+ 契约门禁（复用 `validate_contracts.py` 与 OpenAPI/Prism Mock，pre-commit + CI 双闸门强制）+ 三类高危专项（支付资损 R-04 / 平台封禁 R-类 / 并发幂等）+ 三条 E2E 主链路 + 性能容量基线 + PRD §11 验收映射 + 发布门禁（§28 N4/N6/N7）；显式登记缺口 G1–G4（PIPL 三块延后 / 无障碍待定 / Mock 局限 / Pact 编码期）。§9.4 新增测试计划行标 [闭环]；剩余 #171 部署运维手册 / #170 风险登记表复评。
