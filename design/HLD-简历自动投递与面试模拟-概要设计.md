...
> 2026-08-16 · **v3.33（评审清单闭环 #172–#175：PIPL 合规补充 / 安全威胁模型 / 无障碍定级 / 可观测性 LLD）**：架构师按 `设计评审清单-高级专家软件团队.md` 实施规划任务，优先闭环 🔴 上线前阻断项。新建 `PIPL合规设计补充.md`（§24 自动化决策告知/拒绝开关 + DSAR 四权数据删除链路含 crypto-shred）、`安全威胁模型.md`（STRIDE 六维，密钥/平台凭证攻击面 🔴→已缓解）、`无障碍评估报告.md`（§19 由待定升 WCAG 2.1 AA 定级）、`LLD-可观测性模块-模块设计.md` v1.0（闭合 R-05 日志 schema 落库 + SLI/SLO）。风险登记表 R-14 由「延后」升「设计补充已出，待律师签字」、R-05 由「[LLD 细化]」升「[已缓解-设计态]」。双闸门（契约校验 + PRD↔HLD 防漂移）全绿；PIPL 三块中「律师合规签字」仍为上线前用户决策点，不阻塞编码启动。
...
| §3.11 通知推送模块 LLD | **[闭环]** | 见 `LLD-通知推送模块-模块设计.md` v1.0（渠道降级链/L0–L3 分级/聚合免打扰/去重/隐私） |                                                                                                                                            |
