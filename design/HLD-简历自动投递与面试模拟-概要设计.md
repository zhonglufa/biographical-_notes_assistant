...
> 2026-08-16 · **v3.34（设计粒度加深 A/B/C + 编码期脚手架奠基 D，全选推进）**：按用户「全选」推进 A+B+C+D。A 关键路径 LLD 加深：新建 `时序图规范-关键链路.md`（SEQ-1 投递主链路 / SEQ-2 支付回调 / SEQ-3 通知分发 三张时序图信息说明书，闭合审计缺口①）；`LLD-通知推送模块-模块设计.md` 补 §8 投递语义形式化（at-least-once/dedupKey/幂等/重试退避/DLQ，闭合缺口①）；`LLD-用户与权限模块-模块设计.md` 补 §2.1 角色枚举+权限码表，T-UP-1 升[已闭环-设计态]。B 事件契约补全：新建 `domain-events.event.schema.json`（投递状态/策略变更/支付状态/日报生成/会员权益 五事件 envelope+oneOf 定型 payload），samples 证伪，schema 计数 65→66。C 固化 `设计颗粒度审计报告.md`。D 编码期脚手架第一阶段：contract-first 可运行骨架（见 `scaffold/`）。双闸门全绿。
