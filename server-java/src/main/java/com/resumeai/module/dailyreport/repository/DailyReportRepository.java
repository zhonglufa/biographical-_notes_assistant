...
public interface DailyReportRepository extends BaseMapper<DailyReport> {
    ...
    default DailyReport save(DailyReport e) {
        QueryWrapper<DailyReport> q = new QueryWrapper<DailyReport>()
                .eq("user_id", e.getUserId()).eq("report_date", e.getReportDate());
        if (selectOne(q) != null) {
            update(e, new UpdateWrapper<DailyReport>()
                    .eq("user_id", e.getUserId()).eq("report_date", e.getReportDate()));
        } else {
            insert(e);
        }
