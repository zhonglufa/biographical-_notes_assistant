...
**验收标准**：① Cookie 永不出本机（§C.5）；② 各用户从各自 IP / 设备发起投递，无共享出口 IP（反封禁）；③ 合规责任边界清晰；④ 浏览器算力下沉用户机器，服务端无浏览器集群瓶颈。
...
**验收标准**：未签署免责声明的用户无法进入投递流程；伦理声明在面试模拟入口可见；产品文案不含明示规避平台风控的表述。
...
**验收标准**：采集节奏与投递节奏共用一套频控（§6.1），不触发平台风控（§16.5）。
...
**验收标准**：重装/崩溃后不重复投递（幂等键兜底）；损坏时 fail-safe 不静默放行。
...
**验收标准**：不重复扣款；资损可控（与 §17.1 / §32.1 风险定级一致）。
...
**验收标准**：杜绝脏数据导致错投；降低 LLM / 采集调用控成本（§16）。
...
**设计实现**：HLD §6.5 A。
**验收标准**：破坏性变更经双写/双读过渡 ≥1 周期；旧逻辑不得与新风控/契约冲突导致错投或误封。
...
**设计实现**：HLD §6.5 F。
**验收标准**：弱网/离线不丢关键任务；不自动翻墙（合规红线 §17）。
...
**验收标准**：解析失败有明确兜底路径；质量 SLA 在 v0.9 实测达标；解析成本计入单位经济。
...
**验收标准**：限流失效/进程 hang/自愈失败均有兜底，不拖垮用户机器、不变砖。
