...
- **⚠️ 前端技术栈 truth gap（2026-08-18 用户质询发现 → 待拍板）**：ADR-010（已采纳）与 HLD §2.4 规定 **Vue 3 + Element Plus（PC）+ uni-app（H5/小程序）**；但 `frontend/` 实际实现为 **React 18 + Vite 5 + react-router-dom 6**（`package.json` + `App.jsx` + 各 `screens/*.jsx` 为证），`产品交付结果报告-v2.md` 亦称「React+Vite 骨架」——即技术栈已被替换为 React，却**从未修订 ADR-010 / HLD**，属未记录的技术栈偏离。待用户拍板方向：A）正式采纳 React（修订/覆盖 ADR-010，uni-app 多端列为 MVP 外或延后）/ B）回退 Vue 3 + Element Plus（需重写 `frontend/` 全部组件+路由）。当前 Q11–Q15 续推均按 React 落地，方向未定前先登记、不伪造「已合规」。
...
- **下一工作包**：**V 阶段原型→生产前端转化（ACTIVE）**——A+B+C+U+V+T+O + 护栏1/2/3 已 GOAL REACHED；V 阶段为循环启用后新增待办（`TASK-QUEUE.md` Q9–Q15/Q1），本地不部署。本次 tick 完成 **Q11 = U2 岗位浏览**（A07/A08 生产组件 + 端点路径修正 + A07/A08 mock）。续推队首待办 Q12(U4 策略配置)→Q13(U5 适配器)→Q14(U6 面试)→Q15(U7 支付)→Q1(RAG，用户延后优先级)→Q2-Q4(合规基座，2026-08-18 解锁)。
- **近期推进序列**：Q8(U9) ✅ → Q9(U3) ✅ → Q10(U1) ✅ → Q11(U2) ✅ → Q12(U4) → Q13(U5) → Q14(U6) → Q15(U7) → Q1(RAG 延后) → Q2–Q4(合规基座，R1 解锁)。
