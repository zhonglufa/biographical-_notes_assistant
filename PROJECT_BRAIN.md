...
- 2026-08-18 **ORM 选型 = MyBatis-Plus 3.5.7（用户 Java 工程师拍板）**：替代早期未真正落地的 Spring Data JPA。持久层 `XRepository extends BaseMapper<X>` + `default` 方法保留 JPA 风格签名；`@MapperScan` 限定 `*.repository`；分页 `PaginationInnerInterceptor(MYSQL)`；实体 `@TableName/@TableId/@TableField` 显式映射；测试 schema 由 `src/test/resources/db/migration-test`（H2 兼容副本，剥离 ENGINE/CHARSET/COLLATE、JSON→TEXT、UNSIGNED）提供，生产仍 Flyway V1–V9。
